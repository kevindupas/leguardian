...
class VirtualBracelet:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("command"):
                    ...
                    self.log_action("🔔 COMMAND_RECEIVED", {"command": command, "id": command_id, "status": "📲 PROCESSING"})
                    ...
                    if command == "vibrate_short":
                        ...
                        color = data.get("led_color", "blue")
                        self.led_on(color, command_id)
                    elif command == "led_blink":
                        color = data.get("led_color", "red")
                        pattern = data.get("led_pattern", "fast")
                        self.led_blink(color, pattern, command_id)

                    return data
                else:
                    # Don't log NO_PENDING_COMMANDS to avoid spam
                    return None
            else:
    ...
    def vibrate_short(self, command_id: int):
        ...
        self.log_action("📳 VIBRATE_SHORT", {"pattern": "⚡ 100ms", "status": "✓ EXECUTING"})
        self._acknowledge_command(command_id)

    def vibrate_medium(self, command_id: int):
        """Execute vibration pattern: medium pulse"""
        self.log_action("📳 VIBRATE_MEDIUM", {"pattern": "⚡⚡ 200ms", "status": "✓ EXECUTING"})
        self._acknowledge_command(command_id)

    def vibrate_sos(self, command_id: int):
        """Execute vibration pattern: SOS distress signal"""
        self.log_action("📳 VIBRATE_SOS", {"pattern": "⚡⚡⚡ SOS (100-100-100-200-200-200-100-100-100ms)", "status": "✓ EXECUTING"})
        self._acknowledge_command(command_id)
    ...
    def led_on(self, color: str = "blue", command_id: int = None):
        ...
        self.log_action(f"{color_emoji} LED_ON", {"color": color, "status": "✓ EXECUTING"})
        if command_id:
            self._acknowledge_command(command_id)
    ...
    def led_blink(self, color: str = "red", pattern: str = "fast", command_id: int = None):
        ...
        self.log_action(f"{color_emoji} LED_BLINK", {"color": color, "pattern": f"{pattern_symbol} ({pattern})", "status": "✓ EXECUTING"})
        if command_id:
            self._acknowledge_command(command_id)
...
def simulate_bracelet(bracelet: VirtualBracelet, duration_minutes: int = 5):
    ...
    heartbeat_interval = 120  # seconds (every 2 minutes with location transmission)
    geolocation_update_interval = 30  # seconds (every 30 seconds while in emergency)
